...
int write_bit(FILE *f, int bit) {                                                         //buffer full,needs to be flushed
...
int flush_buffer(FILE *f) {
    if (bits_in_buffer) {
        size_t bytes_written =                                                //buffer contents written
            fwrite(buffer, 1,
                (bits_in_buffer + 7) >> 3, f);           //no.of bits to bytes ..7 addes to ensure remainingbits get rounded to 9
        if (bytes_written < MAX_BUFFER_SIZE && ferror(f))                          //shift by 3=divide by 8
            return -1;
        bits_in_buffer = 0;                                                             //reset
    }
    return 0;
}
...
int read_bit(FILE *f) {
    if (current_bit == bits_in_buffer) {
        if (eof_input)
            ...
            size_t bytes_read =
                fread(buffer, 1, MAX_BUFFER_SIZE, f);                        //read buffer
            if (bytes_read < MAX_BUFFER_SIZE) {
                if (feof(f))
                    eof_input = 1;
            }
            bits_in_buffer = bytes_read << 3;
            current_bit = 0;                              //reset for taking next 4 bytes
        }
    }
...
int write_header(FILE *f) {                              //calculating total size of header
     int i, j, byte = 0,
     ...
     while (j--)
         buffer[byte++] =
             (original_size >> (j << 3)) & 0xff;   //shift bits and extract lowest 8 bits and stores i buffer
     ...
     fwrite(buffer, 1, size, f);                  //once buffer full,we write entire data
     free(buffer);
     return 0;
}

int read_header(FILE *f) {                        //header info
     int i, j, byte = 0, size;                    //track of current position in buffer
     size_t bytes_read;
     ...
     if (bytes_read < 1)
         return END_OF_FILE;
     byte = 0;
     original_size = buff[byte++];                 //store first byte of buff
     ...
     unsigned int weight;
     char *buffer = (char *) calloc(size, 1);                   //allocate memory for buffer
     if (buffer == NULL)
         return MEM_ALLOC_FAIL;
     fread(buffer, 1, size, f);
     byte = 0;
     for (i = 1; i <= num_active; ++i) {                      //iterate over active nodes
